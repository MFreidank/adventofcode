...
def generate_inputs(filepath: Path) -> typing.Iterable[typing.List[typing.Dict[CubeColor, int]]]:
    ...
    """
    cube_regex = r"(?P<num_cubes>\d+) (?P<cube_color>blue|red|green)(?P<separator>,|;|$)"
    with open(filepath) as f:
        for game_id, line in enumerate(f):
            drawn_cubes = [defaultdict(int)] # type: typing.List[typing.Dict[CubeColor, int]]
            for (num_cubes_str, cube_color_str, separator) in re.findall(pattern=cube_regex, string=line):
                drawn_cubes[-1][CubeColor.from_string(cube_color_str)] += int(num_cubes_str)
    ...
    for game in generate_inputs(input_filepath):
        ...
        for turn in game:
            for color, num_cubes in turn.items():
                required_game_specs[color] = max(
                    required_game_specs[color],
                    num_cubes
                )
